...
const validatePassword = (pass) => {
   ...
   schema.is().min(8).is().max(100).has().uppercase().has().lowercase().has().digits(1).has().letters().has().not().spaces();
...
const getFilteredUsersValidation = (req, res) => {
   const Joischema = Joi.object({
      ...
      sortBy: Joi.object({
         ...
         emailClicksCount: Joi.number().valid(-1, 1),
      }),
      limit: Joi.number().min(0),
...
const trackClickValidation = (req, res) => {
   const Joischema = Joi.object({
...
const orderValidation = (req, res) => {
   const Joischema = Joi.object({
      ...
      applyDiscount: Joi.boolean(),
      shouldDeliver: Joi.boolean().when("applyDiscount", {
         is: true,
         then: Joi.valid(true),
      }),
...
const joinCompetitionValidation = (req, res) => {
   const Joischema = Joi.object({
